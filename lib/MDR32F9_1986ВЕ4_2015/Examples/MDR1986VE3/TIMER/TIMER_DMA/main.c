/**
 ******************************************************************************
 * @file    Examples/MDR1986VE3_EVAL/TIMER/TIMER_DMA/main.c
...
/** @addtogroup TIMER_DMA_3T TIMER_DMA
 * @{
 */
...
void main ( void )
{
	...
	RST_CLK_PCLKcmd((RST_CLK_PCLK_RST_CLK | RST_CLK_PCLK_TIMER4
	...
	TIMER_SetChnCompare(MDR_TIMER4, TIMER_CHANNEL1, 0x100);
	...
	TIMER_DMACmd(MDR_TIMER4, TIMER_STATUS_CCR_CAP_CH2, TIMER_DMA_Channel0, ENABLE);

	/* Reset all DMA settings */
	DMA_DeInit();
	DMA_StructInit(&DMA_InitStr);

	/* DMA_Channel_TIM4 configuration ---------------------------------*/
	/* Set Primary Control Data */
	DMA_PriCtrlStr.DMA_SourceBaseAddr = (uint32_t) (&(MDR_TIMER4 ->CCR2));
	...
	/* Init DMA channel */
	DMA_Init(DMA_Channel_REQ_TIM4, &DMA_InitStr);
	...
	while (1) {
		dma_status =
				(DMA_ControlTable[DMA_Channel_REQ_TIM4 ].DMA_Control & 0x07);
		if (dma_status == 0)
			break;
	}
